...
export function initDrillSession(source, target, volume, levels = ['A1', 'A2', 'B1', 'B2', 'C1', 'C2'], mode = 'smart') {
    ...
    if (mode === 'smart') {
        // Concept du Pool Actif : 100 mots max en apprentissage simultané
        const activePoolSize = 100;
        ...
        // Prendre les mots tentés en priorité jusqu'à remplir le pool
        let pool = attemptedWords.slice(0, activePoolSize);

        // Si le pool n'est pas plein, on complète avec de nouveaux mots (aléatoires)
        if (pool.length < activePoolSize) {
            const neededNew = activePoolSize - pool.length;
            const shuffledNew = shuffle([...newWords]);
            pool = [...pool, ...shuffledNew.slice(0, neededNew)];
        }

        // On sélectionne le volume demandé à partir du pool
        selectedWords = shuffle([...pool]).slice(0, volume);
        ...
        selectedWords = validatedWords.slice(0, volume);
        if (selectedWords.length < volume) {
            const remainingNeeded = volume - selectedWords.length;
            const selectedActiveWords = shuffle([...activeWords]).slice(0, remainingNeeded);
            selectedWords = shuffle([...selectedWords, ...selectedActiveWords]);
        }
    }
...
function proceedNextWord(action) {
    ...
    // Le mode révision ne doit jamais dévalider un mot en cas d'erreur
    if (sessionState.mode === 'review') {
        finalStatus = 'validé';
        explicitAttempts = 1;
    }
